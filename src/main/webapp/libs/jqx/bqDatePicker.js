(function($) {
	$.fn.bqDatePicker = function(options) {
		var defaults = {
			name: "dateValue",
			value: null,
			onBlur: null // uživatelský callback
		};
		...
		return this.each(function() {
			...
			$container.data("bqDatePicker", settings);
			...
			var $button = $("<button type='button' class='bq-date-btn'>📅</button>");
			...
			$container.append($input).append($button).append($hidden);
			...
			$input.on("blur", function() {
				...
				if (currentVal !== originalValue) {
					...
					if (typeof settings.onBlur === "function") {
						settings.onBlur.call($container[0], currentVal);
					}
				}
			});
	...
	$(function() {
		$("[data-role='bqDatePicker']").bqDatePicker();
	});
})(jQuery);